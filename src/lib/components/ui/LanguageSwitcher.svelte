<script lang="ts">
    ...
    import { Languages, Check, ChevronDown, ChevronUp, ChevronRight } from 'lucide-svelte';
...
>
    {#if variant === 'menu'}
        <button
            ...
            class="w-full px-4 py-2.5 text-left text-muted transition-colors flex items-center justify-between hover:text-accent hover:bg-border/50 {isOpen ? 'text-accent bg-border/30' : ''}"
        >
    ...
    {:else}
        <button 
            onclick={() => (isOpen = !isOpen)}
            class="flex items-center gap-2 hover:bg-secondary/50 text-xs font-semibold text-muted hover:text-text uppercase tracking-wider px-3 py-1.5 rounded transition-colors"
            aria-label="Change language"
        >
            <Languages size={14} />
            <span>{locales.find(l => l.code === $locale)?.label || 'English'}</span>
            {#if direction === 'up'}
        ...
        >
            {#each locales as l}
                <button
                    ...
                    class="w-full text-left px-4 py-2.5 text-sm hover:bg-secondary/50 transition-colors flex items-center justify-between {l.code === $locale ? 'text-accent font-medium' : 'text-muted hover:text-text'}"
                >
                    <span>{l.label}</span>
                    {#if l.code === $locale}
                        <Check size={14} />
